...
void save_ssaaImage1(const char* filename1, Color** image) {
    FILE* fp = freopen(filename1, "w+", stdout);
    std::cout << "P6\n" << width * ssaaFactor << " " << height * ssaaFactor << "\n255\n";//声明格式
    for (int y = 0; y < ssaaHeight; ++y) {
        for (int x = 0; x < ssaaWidth; ++x) {
            std::cout << image[x][y].r << image[x][y].g << image[x][y].b;
        }
    }
}
void applySSAA(Color** image, Color** ssaaImage) {//超采样的原理就是将原图像的一个                                          
    for (int y = 0; y < height; ++y) {//像素点分配到ssaa图像的多个像素点中,有点类似于放大图像
        for (int x = 0; x < width; ++x) {
            for (int sy = 0; sy < ssaaFactor; ++sy) {
                for (int sx = 0; sx < ssaaFactor; ++sx) {
                    int ssaaX = x * ssaaFactor + sx;
                    int ssaaY = y * ssaaFactor + sy;
                    ssaaImage[ssaaX][ssaaY] = image[x][y];
                }
            }
        }
    }
...
int main() {
    ...
    drawtrigger(x0, y0, x1, y1, x2, y2, Red, image);
    //循环生成100个三角形的图
